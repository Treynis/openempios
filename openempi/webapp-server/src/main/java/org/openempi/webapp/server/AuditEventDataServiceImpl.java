...
public class AuditEventDataServiceImpl extends AbstractRemoteServiceServlet implements AuditEventDataService
{
	...
	public AuditEventListWeb getAuditEventsBySearch( AuditEventSearchCriteriaWeb searchCriteria) throws Exception {
		...
		try {			
			...
			List<AuditEventTypeWeb> auditTypes = new ArrayList<AuditEventTypeWeb>( searchCriteria.getAuditEventTypes() );
	...
	public AuditEventEntryListWeb getAuditEventEntriesBySearch( AuditEventSearchCriteriaWeb searchCriteria ) throws Exception {
		log.debug("Get Audit Event Entries By Search");
		...
		try {			
			...
			// Get total count
			int totalCount = auditEventService.getAuditEventEntryCount(startDate, endDate, auditEventTypeCodes);
			
			// get List of AuditEventEntry
			List<AuditEventEntry> eventEntries = auditEventService.filterAuditEventEntriesPaged(startDate, endDate, auditEventTypeCodes, offset, pageSize );			
			List<AuditEventEntryWeb> eventEntriesWebs = new java.util.ArrayList<AuditEventEntryWeb>(eventEntries.size());
			for (AuditEventEntry ee : eventEntries) {

				AuditEventEntryWeb eew = ModelTransformer.mapAuditEventEntry( ee, AuditEventEntryWeb.class);
				eventEntriesWebs.add(eew);
			}
			
			
			AuditEventEntryListWeb auditList = new AuditEventEntryListWeb();			
			auditList.setTotalCount(totalCount);
			auditList.setAuditEventEntries(eventEntriesWebs);	
